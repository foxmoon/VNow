...
public class IVNowFramework {
	...
	public int setVideoBitrate(int nVidBitrate) {
		if (nVidBitrate < 512) {
			mVideoBitrate = 384;
			mVideoWidth = 320;
			mVideoHeight = 240;
		...
		else if (nVidBitrate < 768) {
			mVideoBitrate = 512;
			mVideoWidth = 640;
			mVideoHeight = 480;
			mVideoFrameRate = 15;
		}
		else if (nVidBitrate < 1024) {
			mVideoBitrate = 768;
			mVideoWidth = 640;
			mVideoHeight = 480;
			mVideoFrameRate = 30;
		}
		else if (nVidBitrate < 2048) {
			mVideoBitrate = 1280;
			mVideoWidth = 1280;
			mVideoHeight = 720;
			mVideoFrameRate = 15;
	...
	public int SetVideoEncoderType(int nVidEncType) {
		if (nVidEncType == 0 || nVidEncType == 1) {
			mVideoEncoderType = nVidEncType;
			return 0;
		}
    ...
    class TestThread implements Runnable {
        public void run() {
        	while(true) {
                ...
                //sendMessage("", "{\"cmd\":\"res_register\", \"result\":\"true\"}");
        	}
        }
    }
}