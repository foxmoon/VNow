...
public class Session {
    ...
    private void initDisplay(Context context) {
        try {
            ...
            if (devWidth > devHeight) {// 玩游戏的时候Session被销毁，重构的时候，可能因为游戏的横屏，导致这里数据异常
                int temp = devHeight;
                devHeight = devWidth;
                devWidth = temp;
            }
    ...
    private void readPreference() {
        ...
        serviceIp = mPreferencesUtils.getString(Constants.MEDIA_SERVER_IP, Constants.DEFULT_SERVER_IP);
    ...
    public void setServiceIp(String ip) {
        serviceIp = ip;
        mPreferencesUtils.putString(Constants.MEDIA_SERVER_IP, ip);
    }
